...
function prefixLines(lines: string[], firstPrefix: string, restPrefix = firstPrefix): string[] {
  ...
  for (const token of tokens) {
    switch (token.type) {
      ...
      case "del":
        ...
        break;
      case "codespan":
        output += chalk.black.bgCyanBright(` ${token.text} `);
        break;
      case "link": {
        const label = renderInline(token.tokens) || token.href;
        const renderedLabel = chalk.underline.cyan(label);
        output += label === token.href
          ? renderedLabel
...
function renderTable(token: Tokens.Table, indent: number): string[] {
  ...
  const separator = chalk.dim(widths.map((width) => "─".repeat(width)).join("─┼─"));
  const renderRow = (cells: string[]) =>
    pad + cells.map((cell, index) => padAnsiEnd(cell, widths[index] ?? 1)).join(chalk.dim(" │ "));
...
function renderList(token: Tokens.List, indent: number): string[] {
  ...
  token.items.forEach((item, index) => {
    ...
    lines.push(...prefixLines(content, pad + bullet, pad + " ".repeat(bullet.length)));
  });
  ...
  switch (token.type) {
    ...
    case "heading": {
      ...
      if (token.depth <= 2) {
        return [
          heading,
          pad + chalk.dim((token.depth === 1 ? "=" : "-").repeat(Math.max(stripAnsi(text).length, 8))),
        ];
      }
    ...
    case "html":
      return (token as Tokens.HTML).text.split("\n").map((line: string) => pad + line);
    default:
