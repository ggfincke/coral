...
// shared ripgrep execution, error handling, & output truncation
...
export function execRipgrep(args: string[], noMatchMessage: string): Promise<ToolResult> {
  return new Promise((resolve) => {
    execFile("rg", args, { timeout: RG_TIMEOUT, maxBuffer: RG_MAX_BUFFER }, (err, stdout, stderr) => {
...
// truncate newline-delimited output to a max number of items
export function truncateOutput(raw: string, maxItems: number, label: string): string {
  ...
  return `${shown}\n\n(Showing ${maxItems} of ${total} ${label} — use a more specific pattern to narrow results)`;
}
