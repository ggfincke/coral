// src/agent/request/attachments.ts
// workspace attachment capture and request-budget rendering
...
// reserve for the largest candidate and cap the resulting UTF-8 allowance
...
): Promise<{ ok: true; path: string } | { ok: false; path: string }>
{
  ...
  {
    signal?.throwIfAborted()
    // keep unreadable paths in the result so callers receive a structured failure
    return { ok: true, path }
  }
}

// capture every approved file before mutating conversation state
// abort the whole capture instead of returning a partial result
...
// render immutable captures so structural text counts against the character budget
...
// fit each mention independently so nested prefixes remain monotonic
...
// strip transient error details before saving the outcome in history
...
// build the complete replacement so callers can commit it atomically
