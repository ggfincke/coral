...
import type { OllamaMessage, OllamaToolCall } from "../ollama/client.js";
...
function estimateMessageTokens(msg: OllamaMessage): number {
...
export function shouldCompact(messages: OllamaMessage[], config: CompactionConfig = DEFAULT_COMPACTION_CONFIG): boolean {
  if (messages.length < config.minMessagesForCompaction) return false;
    ...
    switch (msg.role) {
      ...
      case "assistant":
        ...
        if (msg.tool_calls?.length) {
          for (const call of msg.tool_calls) {
            lines.push(`  [called ${call.function.name}(${JSON.stringify(call.function.arguments)})]`);
          }
        }
        break;
      case "tool":
...
export function buildCompactionPrompt(messagesToSummarize: OllamaMessage[]): string {
