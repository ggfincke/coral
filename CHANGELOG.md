...
- **Undo/redo safety hardening:** replay now revalidates workspace and symlink
  ...
  undo/redo write fails, restores `todo_write` state on undo/redo, and stores
  undo contracts in a neutral type module instead of the replay helper.
  Persisted undo state now omits duplicate live transcript messages, drops whole
  oldest records under an aggregate byte cap, and writes Coral session JSON under
  private POSIX modes.
- **TUI lifecycle polish:** restored and redone `@`-mention prompts now display
  ...
  command palette accepts literal `j`/`k` filter text while arrow keys handle
  navigation.
